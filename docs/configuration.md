...
| Key | Type | Default | Description |
| --- | --- | --- | --- |
| `path_integrand_names` | `str` or list of str | `None` | Per-point quantity (or list of them) integrated along the path. Options: `geodesic`, `pvre`, `pvre_squared`, `vre`, `vre_error`, `pvre_mag`, `E`, `E_mean`, `F_mag`. See [Loss functions](loss-functions.md) for what each one means. |
