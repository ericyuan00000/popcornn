"""Re-run the single-stage pvre baseline on Müller-Brown with knobs
matched to pseudo_huber_winner / huber_winner so the comparison is
apples-to-apples on every axis except the integrand.

The earlier pvre-only run (`mb_pvre_only/`) used n_embed=8 depth=4
(610-param MLP, mismatched to the 18-param MLP used by huber/pseudo)
and threshold=1.0 with only 200 iters — patience-stopped at iter
~100-200 with |g| still 0.4-4.0. Best |F|@TS = 0.75 / 2.33 / 2.14
(mean ~1.74). Not converged.

This rerun:
  - pvre, single-stage
  - lr = 1e-2  (matches huber_winner / pseudo_huber_winner)
  - n_embed = 2, depth = 2  (18 params, same as huber/pseudo)
  - n_iter = 600
  - threshold = 1e-2
  - patience = 10
  - 3 seeds

Output: /pscratch/sd/e/ericyuan/temp/popcornn_huber/mb_pvre_rerun/
"""
import copy
import json
import os
...
OUT_BASE = '/pscratch/sd/e/ericyuan/temp/popcornn_huber/mb_pvre_rerun'
...
def main():
    ...
    for seed in SEEDS:
        out_dir = os.path.join(OUT_BASE, f'pvre_only_s{seed}')
        cfg_path = os.path.join(out_dir, 'config.yaml')
        os.makedirs(out_dir, exist_ok=True)
        write_cfg(base, cfg_path)
        if run_one(cfg_path, seed, out_dir):
            r = analyze(out_dir)
            r.update({'config': 'pvre_only_v2', 'seed': seed,
                      'lr': LR, 'n_iter': N_ITER, 'threshold': THRESHOLD})
            results.append(r)
            with open(os.path.join(OUT_BASE, 'results.json'), 'w') as f:
                json.dump(results, f, indent=2)

    print(f'\n=== pvre-only v2 (3 seeds, single-stage, '
          f'lr={LR:.0e}, 18-param MLP, {N_ITER} iters, threshold={THRESHOLD}) ===',
          flush=True)
    ...
    for r in sorted(results, key=lambda x: x['seed']):
        ...
        print(f'{r["seed"]:>5d} {r["best_f_inf_ts"]:>11.4e} {r["best_iter"]:>10d} '
              f'{r["best_ginf"]:>11.4e} {r["best_barrier"]:>9.4f} '
              f'{r["final_f_inf_ts"]:>11.4e} {ca:>13s} {r["wall_s"]:>8.1f}')
